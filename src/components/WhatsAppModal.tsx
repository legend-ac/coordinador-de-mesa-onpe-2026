...
export const WhatsAppModal: React.FC<WhatsAppModalProps> = ({
  ...
  // Usar info dinámica del coordinador activo
  ...
  const currentMessage = buildWhatsAppMessage(testName, coordinadorInfo);
  const waUrl = getWhatsAppUrl(testPhone, testName, coordinadorInfo);
      ...
      <div className="bg-[#0A111D] text-white rounded-2xl max-w-2xl w-full shadow-2xl border border-[#16253B] overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="bg-[#00223A] border-b border-[#0A111D] text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-[#D31027] rounded-xl">
              <MessageCircle className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-white">Mensaje Oficial WhatsApp</h2>
        ...
        <div className="p-6 space-y-5 bg-[#050912]">
          ...
          <div className="p-4 bg-[#0A111D] rounded-xl border border-[#16253B] space-y-2">
            ...
            <p className="text-sm font-semibold text-white">{perfil.nombreCompleto}</p>
            <p className="text-xs text-slate-400">DNI: {perfil.dni} · {perfil.mesas.join(', ')}</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-slate-300 pt-1">
              ...
              {perfil.oficina && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="truncate">{perfil.oficina}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#0A111D] rounded-xl p-4 shadow-sm text-slate-200 text-xs sm:text-sm whitespace-pre-line leading-relaxed border border-[#16253B] max-h-60 overflow-y-auto">
              {currentMessage}
            </div>
          </div>

          <div className="text-xs text-slate-400 space-y-1">
            <p>✅ El mensaje usa tu nombre y datos de coordinador registrados en el sistema.</p>
            <p>✅ Compatible con WhatsApp normal y WhatsApp Business.</p>
          </div>
        </div>
        ...
        <div className="bg-[#001726] px-6 py-4 border-t border-[#16253B] flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          {waUrl ? (
            ...
            <button
              disabled
              className="w-full sm:w-auto px-5 py-2.5 bg-[#0A111D] text-slate-500 border border-[#16253B] rounded-lg text-xs cursor-not-allowed"
            >
