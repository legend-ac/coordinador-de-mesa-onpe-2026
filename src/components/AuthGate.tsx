...
export const AuthGate: React.FC<AuthGateProps> = ({ children }) => {
  ...
  const [coordinadorPerfil, setCoordinadorPerfil] = useState<CoordinadorPerfil | null>(null);
  ...
  useEffect(() => {
    const storedDni = sessionStorage.getItem('onpe_coordinator_dni');
    const authGranted = sessionStorage.getItem('onpe_auth_granted');

    if (storedDni && authGranted === 'true') {
      loadPerfilAndAuth(storedDni);
    } else {
      setStep('dni');
    }
  }, []);

  const loadPerfilAndAuth = async (dni: string) => {
    try {
      const snap = await getDoc(getPerfilDocRef(dni));
      if (snap.exists()) {
        setCoordinadorPerfil(snap.data() as CoordinadorPerfil);
      } else {
        // Perfil eliminado de Firestore — limpiar sesión
      ...
      // Sin conexión → intentar con cache del sessionStorage
      setStep('dni');
    }
  };
  ...
  const handleDniSubmit = async (e: React.FormEvent) => {
    ...
    if (cleanDni.length < 6) {
      setError('Ingresa un DNI válido (mínimo 6 dígitos).');
      return;
    }
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (pinInput.trim() === foundPerfil.pin) {
        sessionStorage.setItem('onpe_coordinator_dni', foundPerfil.dni);
  ...
  // ── Paso 3: Registro de nuevo coordinador ───────────────────────────────────
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Parsear números de mesa
    const mesasRaw = regMesas
      .split(/[,;\s]+/)
      .map(s => s.trim())
      .filter(s => /^\d+$/.test(s));

    if (mesasRaw.length === 0) {
      setError('Ingresa al menos un número de mesa (ej: 51, 52).');
      return;
    }
    ...
    try {
      ...
      sessionStorage.setItem('onpe_coordinator_dni', cleanDni);
      sessionStorage.setItem('onpe_auth_granted', 'true');
      setCoordinadorPerfil(nuevoPerfil);
    } catch (err) {
      console.error('Error registrando coordinador:', err);
      ...
      <div className="max-w-sm w-full bg-[#0A111D] border border-[#16253B] rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4">
            ...
            <form onSubmit={handleDniSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1.5 text-center">
                  Si es tu primera vez, crearás tu cuenta como coordinador.
                </p>
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-[#D31027] hover:bg-[#B70E22] text-white font-black text-sm rounded-xl shadow-lg cursor-pointer transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          {step === 'pin' && foundPerfil && (
            <form onSubmit={handlePinSubmit} className="space-y-4">
              ...
              <div className="bg-[#00223A] border border-blue-400/30 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-slate-400 truncate">
                    DNI: {foundPerfil.dni} • {foundPerfil.mesas.join(' · ')}
                  </p>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1.5 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-xs flex items-center gap-1 text-slate-400 hover:text-white cursor-pointer"
                  >
              ...
              <button
                type="button"
                onClick={() => { setStep('dni'); setError(null); setPinInput(''); setFoundPerfil(null); }}
            ...
            <form onSubmit={handleRegister} className="space-y-3">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">
                  Mesas asignadas * <span className="text-slate-500 font-normal">(números, ej: 51, 52, 53)</span>
                </label>
                <input
                  type="text"
                  value={regMesas}
                  onChange={(e) => setRegMesas(e.target.value)}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">
                  Oficina Zonal <span className="text-slate-500 font-normal">(opcional)</span>
                </label>
                <input
                  type="text"
                  value={regOficina}
                  onChange={(e) => setRegOficina(e.target.value)}
                  placeholder="Ej: Calle San Martín 207"
                  className="w-full px-3 py-2 bg-[#050912] text-white text-sm rounded-xl border border-[#16253B] focus:border-red-500 focus:outline-none"
                />
              </div>
              ...
              >
                {showRegPin ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                {showRegPin ? 'Ocultar PINs' : 'Ver PINs'}
              </button>
        ...
        <div className="px-6 pb-4 text-center">
          <p className="text-[10px] text-slate-600">
            Oficina Nacional de Procesos Electorales • Perú 2026
          </p>
        </div>
      </div>
    </div>
